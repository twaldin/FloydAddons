// SOURCE: https://github.com/Noamm9/NoammAddons/blob/master/src/main/kotlin/noammaddons/features/impl/misc/Camera.kt
...
object Camera: Feature() {
    @JvmField val smoothSneak = ToggleSetting("Smooth Sneak")
    @JvmField val noNausea = ToggleSetting("Disable Nausea")
    ...
    override fun init() = addSettings(
        SeperatorSetting("Animations"), smoothSneak,
        SeperatorSetting("Clean View"),
        customFov, fov, removeWaterFov, noBlind, noNausea, noPortal, noFaceBlock
    )
    ...
    fun onRenderPortal(event: RenderGameOverlayEvent.Pre) {
        ...
        event.isCanceled = true
        (mc.thePlayer as? EntityAccessor)?.takeIf { it.isInPortal }?.run { setInPortal(false) }
    }
    ...
    object SmoothSneak {
        ...
        private const val animationSpeed = 2.5f
        ...
        private fun updateState(isSneaking: Boolean) {
            if (lastState != isSneaking) { lastState = isSneaking; isAnimationDone = false }
        }
        ...
        private fun finalizeSneaking(): Float { lastX = 1f; isAnimationDone = true; lastOperationTime = 0f; return -SNEAK_OFFSET }
        private fun finalizeUnsneaking(): Float { lastX = 0f; isAnimationDone = true; lastOperationTime = 0f; return 0f }
        private fun getUpY(x: Float): Float = -SNEAK_OFFSET * x * x
        private fun getDownY(x: Float): Float { val adjustedX = x - 1; return SNEAK_OFFSET * adjustedX * adjustedX - SNEAK_OFFSET }
